...
use frame_support::{
	...
	traits::{
		...
		U128CurrencyToVote, Contains, SortedMembers
	},
	weights::{
...
impl SortedMembers<AccountId> for PlatformPotVec {
...
impl SortedMembers<AccountId> for AirdropControllerVec {
	fn sorted_members() -> Vec<AccountId> {
		Self::get()
	}
	#[cfg(feature = "runtime-benchmarks")]
...
impl OnUnbalanced<NegativeImbalance> for DealWithFees {
	fn on_unbalanceds<B>(mut fees_then_tips: impl Iterator<Item = NegativeImbalance>) {
		if let Some(fees) = fees_then_tips.next() {
			...
			// remaining 50% is split in 1% and 49%
...
impl pallet_preimage::Config for Runtime {
	...
	type Event = Event;
	type Currency = Balances;
	type ManagerOrigin = EnsureRootOr3OutOf4Council;
...
parameter_types! {
	...
	pub const UnsignedPhase: u32 = 0; /** DISABLE THIS setting it to 0 AS IT LOCKS THE ERA CHANGING */ // original value: EPOCH_DURATION_IN_BLOCKS / 4;
...
impl pallet_democracy::Config for Runtime {
	...
	type CancellationOrigin = EnsureFullCouncil;
	...
	type CancelProposalOrigin = EnsureOneOf<
		...
		pallet_collective::EnsureProportionAtLeast<_1, _1, AccountId, TechnicalCollective>,
	>;
	type BlacklistOrigin = EnsureRootOr2OutOf3Council;
...
parameter_types! {
	...
	pub const MaximumReasonLength: u32 = 2048;
...
parameter_types! {
	pub const BasicDeposit: Balance = 25 * DOLLARS;       // 258 bytes on-chain
	pub const FieldDeposit: Balance = 500 * CENTS;        // 66 bytes on-chain
	pub const SubAccountDeposit: Balance = 10 * DOLLARS;   // 53 bytes on-chain
...
parameter_types! {
	pub const ConfigDepositBase: Balance = 10 * DOLLARS;
	pub const FriendDepositFactor: Balance = 1 * DOLLARS;
	pub const MaxFriends: u16 = 9;
	pub const RecoveryDeposit: Balance = 50 * DOLLARS;
}
...
impl pallet_uniques::Config for Runtime {
	...
	type InstanceId = u32;
	type Currency = Balances;
	type ForceOrigin = EnsureRootOr3OutOf4Council;
...
construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		Nft: melodity_nft,
		TrackElection: melodity_track_election,
		Bridge: melodity_bridge,
		Airdrop: melodity_airdrop
	}
);
